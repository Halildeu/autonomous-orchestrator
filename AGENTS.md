...
- **Decision Registry (MUST):** Mimari karar gerektiren konuya (auth, DB, UI, altyapı) dokunmadan önce `decisions/registry.v1.json` kontrol et. ACTIVE kararları takip et. Karar değiştirmek istiyorsan **Decision Change Proposal (DCP)** üret — sessizce değiştirme. `rejected_alternatives` listesindeki yaklaşımları tekrar önerme. **Yeni konu kararı alındığında:** `decisions/topics/{topic}.v1.json` oluştur ve `decisions/registry.v1.json`'ı güncelle — `schemas/decision-topic.schema.v1.json` formatında.
