...
def main() -> None:
    ...
    if not manifest_path.exists():
        raise SystemExit("context_orchestration contract_test: FAIL (manifest missing)")
    ...
    if not isinstance(docs_ref, str) or not docs_ref:
        raise SystemExit("context_orchestration contract_test: FAIL (docs_ref missing)")

    docs_path = docs_ref.split("#", 1)[0]
    if docs_path and not (repo_root / docs_path).exists():
        raise SystemExit("context_orchestration contract_test: FAIL (docs_ref path missing)")

    ai_context_refs = manifest.get("ai_context_refs")
    if not isinstance(ai_context_refs, list) or not ai_context_refs:
        raise SystemExit("context_orchestration contract_test: FAIL (ai_context_refs missing)")
    for ref in ai_context_refs:
        if not isinstance(ref, str) or not ref:
            raise SystemExit("context_orchestration contract_test: FAIL (ai_context_refs invalid)")
        if not (repo_root / ref).exists():
            raise SystemExit("context_orchestration contract_test: FAIL (ai_context_refs path missing)")
    ...
    if res.get("status") not in {"OK", "WARN", "IDLE"}:
        raise SystemExit("context_orchestration contract_test: FAIL (extension_run status invalid)")
    if res.get("network_allowed") is not False:
        raise SystemExit("context_orchestration contract_test: FAIL (network must be disabled)")

    print("context_orchestration contract_test: PASS")
