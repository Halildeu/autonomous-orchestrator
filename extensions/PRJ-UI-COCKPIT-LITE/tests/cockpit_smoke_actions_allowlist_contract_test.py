...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            try:
                _post(url, {"op": "github-ops-job-start", "args": {"kind": "SMOKE_FULL"}, "confirm": True})
                raise SystemExit("cockpit_smoke_actions_allowlist_contract_test failed: expected 400 for kind")
            except urllib.error.HTTPError as exc:
                if exc.code != 400:
                    raise SystemExit(f"cockpit_smoke_actions_allowlist_contract_test failed: {exc.code}")

            try:
                _post(url, {"op": "smoke-fast-triage", "confirm": True})
                raise SystemExit("cockpit_smoke_actions_allowlist_contract_test failed: expected 400 for job_id")
            except urllib.error.HTTPError as exc:
                if exc.code != 400:
                    raise SystemExit(f"cockpit_smoke_actions_allowlist_contract_test failed: {exc.code}")
        finally:
