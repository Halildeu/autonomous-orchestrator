...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            url = f"http://127.0.0.1:{port}/api/file?path=../.."
            try:
                urllib.request.urlopen(url, timeout=3)
                raise SystemExit("path_traversal_contract_test failed: expected 400")
            except urllib.error.HTTPError as exc:
                if exc.code != 400:
                    raise SystemExit(f"path_traversal_contract_test failed: {exc.code}")
            url = f"http://127.0.0.1:{port}/api/evidence/raw?path=../.."
            try:
                urllib.request.urlopen(url, timeout=3)
                raise SystemExit("path_traversal_contract_test failed: expected 400 (raw)")
            except urllib.error.HTTPError as exc:
                if exc.code != 400:
                    raise SystemExit(f"path_traversal_contract_test failed: {exc.code} (raw)")
        finally:
