...
def main() -> None:
    ...
    request_id = "REQ-TRACE-LEASE-001"
    manual_request = {
        "version": "v1",
    ...
    if res.get("status") not in {"OK", "WARN"}:
        raise SystemExit("single_trace_lease_contract_test failed: build status")
    ...
    if not lease_path.exists():
        raise SystemExit("single_trace_lease_contract_test failed: lease file missing")
    ...
    if not locked:
        raise SystemExit("single_trace_lease_contract_test failed: LOCKED_ITEM missing")
