...
def cmd_pdca_recheck(args: argparse.Namespace) -> int:
    try:
        dry_run = bool(parse_reaper_bool(str(args.dry_run)))
    except Exception:
        print(json.dumps({"status": "FAIL", "error": "INVALID_DRY_RUN"}, ensure_ascii=False, sort_keys=True))
    ...
    try:
        ...
        payload = run_pdca(workspace_root=workspace_root, dry_run=dry_run)
    except Exception as e:
        print(
            json.dumps(
                {"status": "FAIL", "error_code": "PDCA_INTERNAL_ERROR", "message": str(e)[:200]},
                ensure_ascii=False,
                sort_keys=True,
            )
        )
...
def register_benchmark_subcommands(sub: argparse._SubParsersAction) -> None:
    ...
    ap_pdca = sub.add_parser("pdca-recheck", help="Run PDCA recheck (workspace-root outputs).")
    ap_pdca.add_argument("--workspace-root", required=True)
    ap_pdca.add_argument("--dry-run", default="false", help="true|false (default: false).")
    ap_pdca.set_defaults(func=cmd_pdca_recheck)
