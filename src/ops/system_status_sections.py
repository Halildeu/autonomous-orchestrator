...
def _execution_target_governance_section(workspace_root: Path, *, allow_write: bool) -> dict[str, Any]:
    from src.ops.system_status_sections_governance import _execution_target_governance_section_impl

    return _execution_target_governance_section_impl(workspace_root, allow_write=allow_write)
...
def _module_delivery_section(workspace_root: Path) -> dict[str, Any] | None:
    from src.ops.system_status_sections_module_delivery import _module_delivery_section_impl

    return _module_delivery_section_impl(workspace_root)


def _provider_capability_section(workspace_root: Path) -> dict[str, Any]:
    """Provider capability registry summary for cockpit visibility."""
    repo_root = Path(__file__).resolve().parents[2]
    registry_path = repo_root / "registry" / "provider_capability_registry.v1.json"
    if not registry_path.exists():
        return {"status": "MISSING", "provider_count": 0, "notes": ["registry_not_found"]}
    try:
        data = _load_json(registry_path)
    except Exception as e:
        return {"status": "FAIL", "provider_count": 0, "notes": [f"load_error={e}"]}

    providers = data.get("providers") if isinstance(data.get("providers"), dict) else {}
    capabilities = data.get("capabilities") if isinstance(data.get("capabilities"), list) else []
    summary: list[dict[str, Any]] = []
    for pid, pdata in sorted(providers.items()):
        if not isinstance(pdata, dict):
            continue
        caps = pdata.get("capabilities") if isinstance(pdata.get("capabilities"), dict) else {}
        supported = [c for c, v in caps.items() if v == "supported"]
        experimental = [c for c, v in caps.items() if v == "experimental"]
        summary.append({
            "provider": pid,
            "enabled": bool(pdata.get("enabled")),
            "default_model": str(pdata.get("default_model") or ""),
...
def _role_handoff_section(workspace_root: Path) -> dict[str, Any]:
    """Role-aware handoff and closeout activity summary."""
    reports_dir = workspace_root / ".cache" / "reports"
    closeout_files: list[Path] = []
    handoff_count = 0
    closeout_with_actor = 0
    closeout_without_verify = 0
    ...
    if reports_dir.exists():
        for f in sorted(reports_dir.glob("*.json")):
            ...
            if not isinstance(data, dict):
                continue
            if data.get("version") == "v1" and "trace_meta" in data and "report_path" in data:
                closeout_files.append(f)
                if data.get("actor_role"):
                    closeout_with_actor += 1
                    roles_seen.add(str(data["actor_role"]))
                if data.get("provider"):
                    providers_seen.add(str(data["provider"]))
                if "WARN:VERIFY_RESULT_REF_MISSING" in (data.get("notes") or []):
                    closeout_without_verify += 1
    ...
    return {
        ...
        "closeout_with_actor_count": closeout_with_actor,
        "closeout_without_verify_count": closeout_without_verify,
