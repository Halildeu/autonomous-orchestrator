...
def _require_trace_meta(payload: dict, label: str) -> None:
    ...
    if not isinstance(trace, dict):
        raise SystemExit(f"entrypoint_trace_meta_presence_contract_test failed: {label} trace_meta missing")
    for key in ("version", "work_item_id", "work_item_kind", "run_id", "evidence_paths", "owner_session", "workspace_root"):
        if key not in trace:
            raise SystemExit(f"entrypoint_trace_meta_presence_contract_test failed: {label} trace_meta.{key} missing")
    if not isinstance(trace.get("evidence_paths"), list) or not trace.get("evidence_paths"):
        raise SystemExit(f"entrypoint_trace_meta_presence_contract_test failed: {label} evidence_paths empty")
...
def main() -> None:
    ...
    ws = repo_root / ".cache" / "ws_entrypoint_trace"
    if ws.exists():
        shutil.rmtree(ws)
    ws.mkdir(parents=True, exist_ok=True)

    run_airunner_tick(workspace_root=ws)
    tick_report = json.loads((ws / ".cache" / "reports" / "airunner_tick.v1.json").read_text(encoding="utf-8"))
    _require_trace_meta(tick_report, "airunner_tick")

    request_id = "REQ-TRACE-ENTRY-001"
    manual_request = {
        "version": "v1",
    ...
    exec_report = json.loads((ws / ".cache" / "reports" / "work_intake_exec_ticket.v1.json").read_text(encoding="utf-8"))
    ...
    job_report = json.loads((ws / job_report_rel).read_text(encoding="utf-8"))
    _require_trace_meta(job_report, "github_ops_job")

    run_auto_loop(workspace_root=ws, budget_seconds=60)
    auto_loop_report = json.loads((ws / ".cache" / "reports" / "auto_loop.v1.json").read_text(encoding="utf-8"))
    _require_trace_meta(auto_loop_report, "auto_loop")
