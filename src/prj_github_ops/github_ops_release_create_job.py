...
def _run_release_create_job_impl(
    rc_path: str,
    kind: str,
    token_env: str,
    auth_mode: str,
    ...
    try:
        status_code, body_bytes, headers_get, ssl_selected, ssl_tried = _urlopen_read_with_ssl_fallback(
            get_req,
            workspace_root=ws,
            timeout_seconds=30,
        )
        ...
        if int(status_code or 0) == 200:
            ...
            _write(payload)
            return
        if int(status_code or 0) not in {404, 0}:
            message = f"HTTP_STATUS:{int(status_code or 0)}"
            ...
            payload.update(
                {
                    ...
                    "gh_request_id": _clean_str(headers_get.get("X-GitHub-Request-Id") if hasattr(headers_get, "get") else ""),
                }
            )
