...
def main() -> None:
    ...
    stderr_text = 'Smoke test failed.\\n{"error_code": "TIME_LIMIT"}\\n'
    failure_class, signature_hash = classify_github_ops_failure(stderr_text)
    if failure_class != "POLICY_TIME_LIMIT":
        raise SystemExit("github_ops_failure_time_limit_contract_test failed: class mismatch")

    expected = _hash_text("OTHER|" + "|".join(_extract_lines(stderr_text)))
    if signature_hash != expected:
        raise SystemExit("github_ops_failure_time_limit_contract_test failed: signature mismatch")
    ...
    if failure_class_two != failure_class or signature_hash_two != signature_hash:
        raise SystemExit("github_ops_failure_time_limit_contract_test failed: non-deterministic result")
