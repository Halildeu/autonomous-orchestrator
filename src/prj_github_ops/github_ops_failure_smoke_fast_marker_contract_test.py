...
def main() -> None:
    ...
    if failure_class != "DEMO_CATALOG_MISSING":
        raise SystemExit("github_ops_failure_smoke_fast_marker_contract_test failed: class mismatch")

    expected = _hash_text("DEMO_CATALOG_MISSING|" + "|".join(_extract_lines(stderr_text)))
    if signature_hash != expected:
        raise SystemExit("github_ops_failure_smoke_fast_marker_contract_test failed: signature mismatch")
    ...
    if failure_class_two != failure_class or signature_hash_two != signature_hash:
        raise SystemExit("github_ops_failure_smoke_fast_marker_contract_test failed: non-deterministic result")
