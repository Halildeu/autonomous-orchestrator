...
def main() -> None:
    ...
    ws = repo_root / ".cache" / "ws_operability_docs_hygiene_eval"
    if ws.exists():
        shutil.rmtree(ws)
    ...
    _write_json(
        raw_path,
        {
            ...
            "signals": {
                ...
                "docs_hygiene": {
                    "docs_ops_md_count": 100,
                    "docs_ops_md_bytes": 600000,
                    "repo_md_total_count": 900,
                },
            },
        },
    )
    ...
    if not out_path.exists():
        raise SystemExit("operability_docs_hygiene_eval_contract_test failed: eval output missing")
    ...
    if not isinstance(operability, dict):
        raise SystemExit("operability_docs_hygiene_eval_contract_test failed: operability lens missing")

    reasons = operability.get("reasons")
    if not isinstance(reasons, list):
        raise SystemExit("operability_docs_hygiene_eval_contract_test failed: reasons missing")
    ...
    if not expected.issubset(set(reasons)):
        raise SystemExit("operability_docs_hygiene_eval_contract_test failed: docs_hygiene reasons missing")

    if operability.get("classification") != "FAIL":
        raise SystemExit("operability_docs_hygiene_eval_contract_test failed: classification not FAIL")
