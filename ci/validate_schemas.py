...
def validate_roadmaps(repo_root: Path) -> tuple[int, int]:
    ...
    if not schema_path.exists():
        print("WARN: schemas/roadmap.schema.json not found; skipping roadmap validation.")
        return (0, 0)

    roadmaps_dir = repo_root / "roadmaps"
    if not roadmaps_dir.exists():
        print("WARN: roadmaps/ not found; skipping roadmap validation.")
        return (0, 0)

    validator = Draft202012Validator(load_json(schema_path))
    roadmap_paths = iter_sorted([p for p in roadmaps_dir.rglob("roadmap.v*.json") if p.is_file()])
    if not roadmap_paths:
        print("WARN: No roadmap files found under roadmaps/**/roadmap.v*.json; skipping.")
        return (0, 0)
...
def validate_roadmap_changes(repo_root: Path) -> tuple[int, int]:
    ...
    if not schema_path.exists():
        print("WARN: schemas/roadmap-change.schema.json not found; skipping roadmap change validation.")
        return (0, 0)

    changes_dir = repo_root / "roadmaps" / "SSOT" / "changes"
    if not changes_dir.exists():
        print("WARN: roadmaps/SSOT/changes not found; skipping roadmap change validation.")
        return (0, 0)
...
def validate_debt_changes(repo_root: Path) -> tuple[int, int]:
    ...
    if not schema_path.exists():
        print("WARN: schemas/chg-debt.schema.json not found; skipping debt CHG validation.")
        return (0, 0)
    if not changes_dir.exists():
        print("WARN: roadmaps/SSOT/changes/debt not found; skipping debt CHG validation.")
        return (0, 0)
...
def validate_formats(repo_root: Path) -> tuple[int, int]:
    ...
    if not schema_path.exists():
        print("WARN: schemas/format-autopilot-chat.schema.json not found; skipping format validation.")
        return (0, 0)

    formats_dir = repo_root / "formats"
    if not formats_dir.exists():
        print("WARN: formats/ not found; skipping format validation.")
        return (0, 0)

    format_paths = iter_sorted([p for p in formats_dir.glob("format-autopilot-chat.v*.json") if p.is_file()])
    if not format_paths:
        print("WARN: No format files found at formats/format-autopilot-chat.v*.json; skipping.")
        return (0, 0)
...
def validate_packs(repo_root: Path) -> tuple[int, int]:
    ...
    if not schema_path.exists():
        print("WARN: schemas/pack-manifest.schema.v1.json not found; skipping pack validation.")
        return (0, 0)

    packs_dir = repo_root / "packs"
    if not packs_dir.exists():
        print("WARN: packs/ not found; skipping pack validation.")
        return (0, 0)

    pack_paths = iter_sorted([p for p in packs_dir.rglob("pack.manifest.v1.json") if p.is_file()])
    if not pack_paths:
        print("WARN: No pack manifests found under packs/**/pack.manifest.v1.json; skipping.")
        return (0, 0)
...
def validate_project_manifests(repo_root: Path) -> tuple[int, int]:
    ...
    if not schema_path.exists():
        print("WARN: schemas/project-manifest.schema.json not found; skipping project manifest validation.")
        return (0, 0)

    projects_dir = repo_root / "roadmaps" / "PROJECTS"
    if not projects_dir.exists():
        print("WARN: roadmaps/PROJECTS not found; skipping project manifest validation.")
        return (0, 0)

    manifest_paths = iter_sorted([p for p in projects_dir.rglob("project.manifest.v1.json") if p.is_file()])
    if not manifest_paths:
        print("WARN: No project manifests found under roadmaps/PROJECTS/**/project.manifest.v1.json; skipping.")
        return (0, 0)
...
def validate_capabilities(repo_root: Path) -> tuple[int, int]:
    ...
    if not schema_path.exists():
        print("WARN: schemas/spec-capability.schema.json not found; skipping capability validation.")
        return (0, 0)

    caps_dir = repo_root / "capabilities"
    if not caps_dir.exists():
        print("WARN: capabilities/ not found; skipping capability validation.")
        return (0, 0)
...
def validate_repo_layout(repo_root: Path) -> tuple[int, int]:
    ...
    if not schema_path.exists():
        print("WARN: schemas/repo-layout.schema.json not found; skipping repo layout validation.")
        return (0, 0)
    if not layout_path.exists():
        print("WARN: docs/OPERATIONS/repo-layout.v1.json not found; skipping repo layout validation.")
        return (0, 0)
