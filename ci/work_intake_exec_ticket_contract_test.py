...
def main() -> None:
    ...
    baseline_counts = (
        int(report.get("applied_count") or 0),
        int(report.get("planned_count") or 0),
        int(report.get("idle_count") or 0),
    )
    ...
    counts2 = (
        int(report2.get("applied_count") or 0),
        int(report2.get("planned_count") or 0),
        int(report2.get("idle_count") or 0),
    )
    if order2 != baseline_order or counts2 != baseline_counts:
        raise SystemExit("work_intake_exec_ticket_contract_test failed: ordering or counts not deterministic.")
